...
/**
 * 
 * <h3><b>OpenParkProgramInfo</b></h3></br>
 * 
 * 공공정보명 : 공원정보<br>
 * 서비스명 : 공원 프로그램 정보조회<br>
 * 서비스명 : 프로그램ID별 공원 프로그램 정보조회<br>
 * 서비스명 : 공원ID별 공원 프로그램 정보조회<br>
 * 서비스명 : 프로그램명별 공원 프로그램 정보조회<br>
 * 서비스명 : 공원명별 공원 프로그램 정보조회<br>
 * 서비스명 : 주소별 공원 프로그램 정보조회<br>
 * 분류체계 : 환경>공원녹지
 * <p>
 * 
 ...
 * @since 1.0.0
 */
public class OpenParkProgramInfo extends OpenBase
{
	/**
	 * 결과데이터
	 */
	public Collection<ParkProgramInfo> infos;
	...
	{
		infos = new ArrayList<ParkProgramInfo>();
		...
		while (it.hasNext())
		{
			...
			// Logger.v("row = " + rowId + "\n" + row.toString());
			ParkProgramInfo info = (ParkProgramInfo) manager.deserialize(row.toString(), ParkProgramInfo.class);
			info.rowId = rowId;
			infos.add(info);
		}
	}
