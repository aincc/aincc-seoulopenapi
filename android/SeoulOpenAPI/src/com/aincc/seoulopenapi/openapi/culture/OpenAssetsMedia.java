...
/**
 * 
 * <h3><b>OpenAssetsMedia</b></h3></br>
 * 
 * 공공정보명 : 문화정보<br>
 * 서비스명 : 문화재 이미지_파일 제공 <br>
 * 분류체계 : 문화관광>문화
 * 
 ...
 * @since 1.0.0
 */
public class OpenAssetsMedia extends OpenBase
{
	/**
	 * 결과데이터
	 */
	public Collection<AssetsMedia> infos;
	...
	{
		infos = new ArrayList<AssetsMedia>();
		...
		while (it.hasNext())
		{
			...
			// Logger.v("row = " + rowId + "\n" + row.toString());
			AssetsMedia info = (AssetsMedia) manager.deserialize(row.toString(), AssetsMedia.class);
			info.rowId = rowId;
			infos.add(info);
		}
	}
